...
import AttributeCreate from "@/components/Dashboard/Admin/Attribute/AttributeCreate";
import AttributeEdit from "@/components/Dashboard/Admin/Attribute/AttributeEdit";
...
import { getColors } from "@/redux/services/theme/themeSlice";
...
const Attribute = () => {
  ...
  const { data: attributes, isFetching } = useGetAttributesQuery({
    page: currentPage,
    limit: pageSize,
    search,
  });

  const { data: attributeData } = useGetSingleAttributeQuery(itemId, {
    skip: !itemId,
  });
  ...
  const [deleteBulkAttribute] = useDeleteBulkAttributeMutation();
  ...
  const tableData = attributes?.results?.map((item) => ({
    key: item._id,
    name: item?.name,
    options: item?.options?.map((item) => (
      <Tag key={item?._id} color={primaryColor}>
        {item?.name}
      </Tag>
    )),
  ...
  return (
    <div className="px-5">
      <TableHeader
        ...
        deleteBulk={deleteBulkAttribute}
      ...
      <Pagination
        ...
        total={attributes?.meta?.totalCount}
        current={currentPage}
      ...
      <AttributeCreate open={open} setOpen={setOpen} />
      <AttributeEdit itemId={itemId} open={openEdit} setOpen={setOpenEdit} />
      <DetailsModal
        itemId={itemId}
